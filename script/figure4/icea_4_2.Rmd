...
### 4.4.1 Filter and organize input
Order
```{r}
#T
...
cairo_pdf("output/plots/figure4/Tgene_alternative_promoters_circos_3.pdf", width = 12, height=6)
layout(matrix(1:3, 1, 3))
par(mar = c(0.5, 0.5, 0.5, 0.5))

#p2c2
circos.clear()
circos.par("track.height" = 0.1, "clock.wise"=F, start.degree=15, gap.degree=c(30, 5,5,30))
...
Plot -B
OBS: During emrge make sure, interaktions between pseduo/prom are removed (when B element is also T pseudo)
```{r}
#Oranize for circos
...
cairo_pdf("output/plots/figure4/Bgene_alternative_promoters_circos_3.pdf", width = 12, height=6)
layout(matrix(1:3, 1, 3))
par(mar = c(0.5, 0.5, 0.5, 0.5))

#p2c2
circos.clear()
circos.par("track.height" = 0.1, "clock.wise"=F, start.degree=-8, gap.degree=c(30, 5,5,30))
...
## 4.5 Changes in gene expression at B/T genes in EKOvsWT

### 4.5.1 Load and organize RNA-seq data from WT/EBf1KO
```{r}
...
### 4.5.2 Merge with interaction data
```{r}
#T
...
T.int$group1 <- ifelse(T.int$Tgene_WT=="tgene", "Common", "EKO_only")
T.int$group2 <- ifelse(T.int$tss_EKO=="alt", "Common", "WT_only")
T.int <- T.int[T.int$tss_WT=="alt",]
T.int.ebf1 <- merge(T.int, rna.norm.WT.EKO[,c(3,10:12)], by.x="Gene.x", by.y="symbol")
T.int.ebf1 <- T.int.ebf1[,c(1,12,13,14,15,16)]
T.int.ebf1 <- T.int.ebf1[!duplicated(T.int.ebf1),]
T.int.ebf1.wide <- gather(T.int.ebf1, celltype, value, WT_mean:EKO_mean, factor_key=TRUE)
T.int.ebf1.wide$celltype <- factor(T.int.ebf1.wide$celltype, levels=c("EKO_mean", "WT_mean"))
...
## 4.6 Generate tracks files for T element interactions

### 4.6.1 Filter interaction
```{r}
