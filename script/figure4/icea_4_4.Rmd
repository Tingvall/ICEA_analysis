...
cairo_pdf("output/plots/figure4/BTgene_alternative_promoters_venn.pdf", width = 12, height=6)
venn_gene_t+venn_int_t+venn_alt_t+venn_gene_b+venn_int_b+venn_alt_b
dev.off()
```


## 4.5 Generate tracks files for B/T element interactions

### 4.5.1 Filtered interaction
B/T elements
```{r}
#P2C2
...
```{r}
#P2C2
int.p2c2.f <- int.p2c2[int.p2c2$Q.Value_Bias <= 10^-(int.p2c2.cutoff),]

int.p2c2.longrange <- rbind(data.frame(chr=int.p2c2.f$chr1, start=int.p2c2.f$s1, end=int.p2c2.f$e1, loc2=paste0(int.p2c2.f$chr2, ":", int.p2c2.f$s2, "-", int.p2c2.f$e2, ",", -log10(int.p2c2.f$Q.Value_Bias))),data.frame(chr=int.p2c2.f$chr2, start=int.p2c2.f$s2, end=int.p2c2.f$e2, loc2=paste0(int.p2c2.f$chr1, ":", int.p2c2.f$s1, "-", int.p2c2.f$e1, ",", -log10(int.p2c2.f$Q.Value_Bias))))
int.p2c2.longrange <- int.p2c2.longrange[!duplicated(int.p2c2.longrange),]
write.table(int.p2c2.longrange, file = "output/data/figure4/interactions/filtered_interactions_p2c2.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)


#WT
int.wt.f <- int.wt[int.wt$Q.Value_Bias <= 10^-(int.wt.cutoff),]

int.wt.longrange <- rbind(data.frame(chr=int.wt.f$chr1, start=int.wt.f$s1, end=int.wt.f$e1, loc2=paste0(int.wt.f$chr2, ":", int.wt.f$s2, "-", int.wt.f$e2, ",", -log10(int.wt.f$Q.Value_Bias))),data.frame(chr=int.wt.f$chr2, start=int.wt.f$s2, end=int.wt.f$e2, loc2=paste0(int.wt.f$chr1, ":", int.wt.f$s1, "-", int.wt.f$e1, ",", -log10(int.wt.f$Q.Value_Bias))))
int.wt.longrange <- int.wt.longrange[!duplicated(int.wt.longrange),]
write.table(int.wt.longrange, file = "output/data/figure4/interactions/filtered_interactions_wt.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)

#EKO
int.eko.f <- int.eko[int.eko$Q.Value_Bias <= 10^-(int.eko.cutoff),]

int.eko.longrange <- rbind(data.frame(chr=int.eko.f$chr1, start=int.eko.f$s1, end=int.eko.f$e1, loc2=paste0(int.eko.f$chr2, ":", int.eko.f$s2, "-", int.eko.f$e2, ",", -log10(int.eko.f$Q.Value_Bias))),data.frame(chr=int.eko.f$chr2, start=int.eko.f$s2, end=int.eko.f$e2, loc2=paste0(int.eko.f$chr1, ":", int.eko.f$s1, "-", int.eko.f$e1, ",", -log10(int.eko.f$Q.Value_Bias))))
int.eko.longrange <- int.eko.longrange[!duplicated(int.eko.longrange),]
write.table(int.eko.longrange, file = "output/data/figure4/interactions/filtered_interactions_eko.bed", quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
```
...
#p2c2
sort -k1,1 -k2,2n filtered_interactions_p2c2.bed > filtered_interactions_p2c2_2.bed
bgzip filtered_interactions_p2c2_2.bed
tabix -p bed filtered_interactions_p2c2_2.bed.gz

#eko
sort -k1,1 -k2,2n filtered_interactions_eko.bed > filtered_interactions_eko_s.bed
bgzip filtered_interactions_eko_s.bed
tabix -p bed filtered_interactions_eko_s.bed.gz

#wt
sort -k1,1 -k2,2n filtered_interactions_wt.bed > filtered_interactions_wt_s.bed
bgzip filtered_interactions_wt_s.bed
tabix -p bed filtered_interactions_wt_s.bed.gz
```
