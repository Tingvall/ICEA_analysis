...
files.rna.immgen <- list.files(path = "/mnt/data/common/tobias/tg/rnaseq/Immgen/counts", pattern = "genes.results$", full.names = T)
names(files.rna.immgen) <- sub(".Aligned.toTranscriptome.rsem.out.genes.results", "",list.files(path = "/mnt/data/common/tobias/tg/rnaseq/Immgen/counts", pattern = "genes.results$"))
names(files.rna.immgen) <- c(names(files.rna.immgen[1:17]), "LTHSC_34neg_1", "LTHSC_34neg_2", "LTHSC_34pos_1", "LTHSC_34pos_2", names(files.rna.immgen[22:25]))
files.rna.immgen <- files.rna.immgen[c(18:21, 24:25, 22:23, 1:2, 10:17, 3:9)]
...
#Create coldata
coldata.rna.immgen <- data.frame(sampleID=colnames(counts.rna.immgen), celltype=c(rep(c("LTHSC_34neg","LTHSC_34pos","STHSC","MPP4","CLP","FrA","FrBC", "FrD","FrE", "DN1"),each=2),"DN2a", rep(c("DN2b", "DN3"), each=2)), replicate=c(rep(c(1,2), 10), 1, 1,2,1,2))
coldata.rna.immgen$celltype <- factor(coldata.rna.immgen$celltype, levels = c("LTHSC_34neg","LTHSC_34pos","STHSC","MPP4","CLP","FrA","FrBC", "FrD","FrE", "DN1","DN2a", "DN2b", "DN3"))
...
DESeq2_fun(counts = counts.rna.immgen, coldata = coldata.rna.immgen, selection = c(13,14,22,23), factor="celltype", groups = c("DN2b", "FrBC"), filt=T, filt_n = 2, filt_value = 10, mapID = T, orgdb = org.Mm.eg.db)
...
vst.rna.immgen.mean <- data.frame(LTHSC_34neg=rowMeans(vst.rna.immgen.df[,1:2]), LTHSC_34pos=rowMeans(vst.rna.immgen.df[,3:4]), STHSC=rowMeans(vst.rna.immgen.df[,5:6]), MPP4=rowMeans(vst.rna.immgen.df[,7:8]),CLP=rowMeans(vst.rna.immgen.df[,9:10]),FrA=rowMeans(vst.rna.immgen.df[,11:12]), FrBC=rowMeans(vst.rna.immgen.df[,13:14]), FrD=rowMeans(vst.rna.immgen.df[,15:16]), FrE=rowMeans(vst.rna.immgen.df[,17:18]), DN1=rowMeans(vst.rna.immgen.df[,19:20]),DN2a=vst.rna.immgen.df[,21], DN2b=rowMeans(vst.rna.immgen.df[,22:23]), DN3=rowMeans(vst.rna.immgen.df[,24:25]))
vst.rna.immgen.z <- as.data.frame(vst.rna.immgen.mean-rowMeans(vst.rna.immgen.mean))/rowSds(as.matrix(vst.rna.immgen.mean))
vst.rna.immgen.mean <- data.frame(LTHSC_34neg=rowMeans(vst.rna.immgen.df[,1:2]), LTHSC_34pos=rowMeans(vst.rna.immgen.df[,3:4]), STHSC=rowMeans(vst.rna.immgen.df[,5:6]), MPP4=rowMeans(vst.rna.immgen.df[,7:8]),CLP=rowMeans(vst.rna.immgen.df[,9:10]),FrA=rowMeans(vst.rna.immgen.df[,11:12]), FrBC=rowMeans(vst.rna.immgen.df[,13:14]), FrE=rowMeans(vst.rna.immgen.df[,17:18]), DN1=rowMeans(vst.rna.immgen.df[,19:20]),DN2a=vst.rna.immgen.df[,21], DN2b=rowMeans(vst.rna.immgen.df[,22:23]), DN3=rowMeans(vst.rna.immgen.df[,24:25]))
vst.rna.immgen.z <- as.data.frame(vst.rna.immgen.mean-rowMeans(vst.rna.immgen.mean))/rowSds(as.matrix(vst.rna.immgen.mean))
...
groups = c(rep("Common progenitors", 4), rep( "B cell progenitors",5), rep( "T cell progenitors",4))
...
BT_elements_anno_ICEA230238 <- BT_elements_anno_ICEA230238[,c(17,9,5,18, 10,13)]
...
BT_elements_anno_ICEAP2C2 <- BT_elements_anno_ICEAP2C2[,c(17,9,5,18, 10,13)]
...
BT.elements.ICEA$type_anno <- ifelse(BT.elements.ICEA$Peak_type=="Promoter", "Promoter", ifelse(BT.elements.ICEA$Annotation_method=="Proximal_anno", "Proximal","Interaction" ))

#stats for revision
table(BT.elements.ICEA$type_anno, BT.elements.ICEA$group)
#B: 15.22% Proximal anno
#T: 18.54% Priximal annp

BT.elements.ICEA.anno.type.B <- unique(BT.elements.ICEA[BT.elements.ICEA$group=="B" & BT.elements.ICEA$Peak_type!="Promoter",c(1,7)])
BT.elements.ICEA.anno.type.B$type <- ifelse(BT.elements.ICEA.anno.type.B$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="B" & BT.elements.ICEA$Annotation_method=="Interaction_anno",]$peakID & BT.elements.ICEA.anno.type.B$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="B" & BT.elements.ICEA$Annotation_method=="Proximal_anno",]$peakID, "Both", ifelse( BT.elements.ICEA.anno.type.B$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="B" & BT.elements.ICEA$Annotation_method=="Interaction_anno",]$peakID, "Interaction", "Promimity"))

BT.elements.ICEA.anno.type.T <- unique(BT.elements.ICEA[BT.elements.ICEA$group=="T" & BT.elements.ICEA$Peak_type!="Promoter",c(1,7)])
BT.elements.ICEA.anno.type.T$type <- ifelse(BT.elements.ICEA.anno.type.T$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="T" & BT.elements.ICEA$Annotation_method=="Interaction_anno",]$peakID & BT.elements.ICEA.anno.type.T$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="T" & BT.elements.ICEA$Annotation_method=="Proximal_anno",]$peakID, "Both", ifelse( BT.elements.ICEA.anno.type.T$peakID %in% BT.elements.ICEA[BT.elements.ICEA$group=="T" & BT.elements.ICEA$Annotation_method=="Interaction_anno",]$peakID, "Interaction", "Promimity"))

BT.elements.ICEA.anno.type <- rbind(BT.elements.ICEA.anno.type.B, BT.elements.ICEA.anno.type.T)

table(BT.elements.ICEA.anno.type$group, BT.elements.ICEA.anno.type$type)
```
...
Violin plot of atac levels in non-promoter peaks: Grouped on annotation methhod
```{r}
BT.elements.ICEA.anno.type <- merge(BT.elements.ICEA.anno.type, BT.elements.atac.filt[,c("peakID", "ATAC", "celltype")], by="peakID")
BT.elements.ICEA.anno.type$type <- factor(BT.elements.ICEA.anno.type$type, levels=c("Promimity", "Both", "Interaction"))

my_comparisons <- list( c("230238", "P2C2"))
BT_element_atac.ann <- ggplot(BT.elements.ICEA.anno.type, aes(celltype,log2(ATAC+1), fill=celltype))+
  geom_violin()+ geom_boxplot(width=0.1, outlier.alpha = 0)+
  theme_bw() + theme(panel.border = element_blank(), panel.grid.major = element_blank(),
panel.grid.minor = element_blank(), axis.line = element_line(colour = "black"))+scale_fill_manual(values=c(rep(c("white","grey"), 6)))+
...
ggsave(path="output/plots/figure2", filename="BT_element_ATAC_annotation_method.pdf", plot = BT_element_atac.ann, device = "pdf", dpi=300, width = 6, height = 4)
```
