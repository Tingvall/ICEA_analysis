...
#guide.info <- guide.info[guide.info$type!="single" & guide.info$gene !="Myl4" & guide.info$Exp !="249",]
...
#guide.info.B.n<- data.frame(guide.info.B[,1:3], rbind(mapply('/', guide.info.B[guide.info.B$condition=="OP9",4:ncol(guide.info.B)], guide.info.B[guide.info.B$guide=="sgR26" & guide.info.B$condition=="OP9",4:ncol(guide.info.B)]), mapply('/', guide.info.B[guide.info.B$condition=="DL1",4:ncol(guide.info.B)], guide.info.B[guide.info.B$guide=="sgR26" & guide.info.B$condition=="DL1",4:ncol(guide.info.B)])))
...
```{r}
library(dplyr)
guide.info.pool <- melt(setDT(guide.info.B), id.vars = colnames(guide.info.B[,1:4]), variable.name = "rep")
guide.info.pool <- data.frame(guide.info.pool[!is.na(guide.info.pool$value)])
guide.info.pool <- merge(guide.info.pool, unique(EBF1.anno.Bdev[,c(1,5)]), by.x="gene", by.y="Gene",all.x=T)
guide.info.pool <- guide.info.pool[order(guide.info.pool$Expression_WTvsEKO, decreasing = T, na.last = F),]
guide.info.pool <- guide.info.pool[guide.info.pool$gene %in% c(EBF1.anno.Bdev$Gene, "R26"),]
guide.info.pool$condition <- factor(guide.info.pool$condition, levels=c("OP9", "DL1"))

#Relative to R26
R26.median  <- guide.info.pool[guide.info.pool$gene=="R26",] %>% group_by(condition) %>% summarise_at(vars(value), list(median = median))
guide.info.pool$FC <- ifelse(guide.info.pool$condition =="OP9", guide.info.pool$value/R26.median[R26.mean$condition=="OP9",]$median, guide.info.pool$value/R26.median[R26.mean$condition=="DL1",]$median)
guide.info.pool$log2FC <- log2(guide.info.pool$FC)
guide.info.pool$name <- paste0(guide.info.pool$gene, "_",guide.info.pool$guide)

#plot
source("script/general/r_functions/TG_theme.R")
guide.plot <- ggboxplot(guide.info.pool, x = "gene", y = "value", fill = "Expression_WTvsEKO",  color= "black")+
  #geom_hline(yintercept = mean(guide.info.pool[guide.info.pool$gene=="R26", "value"]), linetype = 2)+ 
  scale_fill_gradientn(colours=c("#1f426a","#cbcad7",'white','white',"#dcccdf", "#93302f"),breaks=c(-10, -5,0,5, 10), values=scales::rescale(c(-10,-log2(1.5)-0.01, -log2(1.5), log2(1.5), log2(1.5)+0.01, 10)),limits=c(-12, 12))+
  stat_compare_means(label = "p.signif", ref.group = "R26", hide.ns = TRUE)+
  TG_theme()+xlab("") + ylab("B cell count")+facet_grid(condition~.)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))

guide.plot <- ggboxplot(guide.info.pool, x = "gene", y = "FC", fill = "Expression_WTvsEKO",  color= "black")+
  #geom_hline(yintercept = mean(guide.info.pool[guide.info.pool$gene=="R26", "FC"]), linetype = 2)+ 
  scale_fill_gradientn(colours=c("#1f426a","#cbcad7",'white','white',"#dcccdf", "#93302f"),breaks=c(-10, -5,0,5, 10), values=scales::rescale(c(-10,-log2(1.5)-0.01, -log2(1.5), log2(1.5), log2(1.5)+0.01, 10)),limits=c(-12, 12))+
  stat_compare_means(label = "p.signif",  ref.group = "R26", hide.ns = TRUE)+
  TG_theme()+xlab("") + ylab("B cell count")+facet_grid(condition~.)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))

guide.plot <- ggboxplot(guide.info.pool, x = "gene", y = "log2FC", fill = "Expression_WTvsEKO",  color= "black")+
  #geom_hline(yintercept = mean(guide.info.pool[guide.info.pool$gene=="R26", "log2FC"]), linetype = 2)+ 
  scale_fill_gradientn(colours=c("#1f426a","#cbcad7",'white','white',"#dcccdf", "#93302f"),breaks=c(-10, -5,0,5, 10), values=scales::rescale(c(-10,-log2(1.5)-0.01, -log2(1.5), log2(1.5), log2(1.5)+0.01, 10)),limits=c(-12, 12))+
  stat_compare_means(label = "p.signif",  ref.group = "R26", hide.ns = TRUE)+
  TG_theme()+xlab("") + ylab("B cell count")+facet_grid(condition~.)+
    scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))

guide.plot <- ggboxplot(guide.info.pool, x = "name", y = "log2FC", fill = "Expression_WTvsEKO",  color= "black")+
  #geom_hline(yintercept = mean(guide.info.pool[guide.info.pool$gene=="R26", "log2FC"]), linetype = 2)+ 
  scale_fill_gradientn(colours=c("#1f426a","#cbcad7",'white','white',"#dcccdf", "#93302f"),breaks=c(-10, -5,0,5, 10), values=scales::rescale(c(-10,-log2(1.5)-0.01, -log2(1.5), log2(1.5), log2(1.5)+0.01, 10)),limits=c(-12, 12))+
  stat_compare_means(label = "p.signif",  ref.group = "R26 sgR26", hide.ns = TRUE)+
  TG_theme()+xlab("") + ylab("B cell count")+facet_grid(condition~.)+
    scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))
