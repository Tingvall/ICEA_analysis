...
Loading packages
```{r}
packages <- c("readxl","AnnotationDbi","org.Mm.eg.db", "clusterProfiler", "ggplot2", "ggpubr", "circlize", "DESeq2", "DiffBind", "ComplexHeatmap", "BiocParallel", "data.table", "tidyr", "dplyr")
...
EBF1.anno.exp.pd <- rbind(unique(EBF1.anno.exp.p[,c(1,5,7)]),unique(EBF1.anno.exp.d[,c(1,5,7)]))
write.table(EBF1.anno.exp.pd, "output/data/figure3/Ebf1_bound.txt", quote = F, sep = "\t", row.names = F, col.names = T)

```


## 3.2 Ubiqutous genes associated with B/T cell growth (Tinas data)
...
guide.info <- read.table("data/other/Bdev_guides/Lineage_count_guides.txt", header=TRUE, sep="\t")
guide.info <- guide.info[!guide.info$guide %in% c("sg19", "sg2","sg69-70", "sg45" ),]
guide.info <- aggregate(cbind(NK,Dendritic, Myeloid, B_cell, T_cell, Sum) ~ Exp + guide + type + gene + condition, data = guide.info, FUN = mean)
guide.info.B <- guide.info[,c(1:5, 9)] %>% spread(key = "Exp", value = "B_cell")
guide.info.B <- guide.info.B[order(guide.info.B$condition, decreasing = T),]
guide.info.T <- guide.info[,c(1:5, 10)] %>% spread(key = "Exp", value = "T_cell")
guide.info.T <- guide.info.T[order(guide.info.T$condition, decreasing = T),]

#Extracting normalized gene expression for WT/EKO
EBF1.anno.Bdev <- merge(guide.info.B, rna.norm.WT.EKO, by.x="gene", by.y="symbol", all.x=T)
EBF1.anno.Bdev <- EBF1.anno[EBF1.anno$Gene %in% guide.info.B$gene,]
EBF1.anno.Bdev <- merge(merge(EBF1.anno.Bdev, rna.norm.WT.EKO[,c(3,10:12)], by.x="Gene", by.y="symbol"), guide.info.B[,1:4], by.x="Gene", by.y="gene")
EBF1.anno.Bdev <- EBF1.anno.Bdev[,c(1,3,8:13,2,4:7)]
colnames(EBF1.anno.Bdev) <- c("Gene", "entrezID", "Expression_WT", "Expression_EKO", "Expression_WTvsEKO","guide", "type", "condition","peakID", "DistanceTSS", "PeakType", "Annotation", "InteractionScore")
```


### 3.2.2 B dev plot
Plot of realtive B/T cell count for guide pools (one per gene), color coded for WTvsEKO
```{r}
#Combine
guide.info.B <- melt(setDT(guide.info.B), id.vars = colnames(guide.info.B[,1:4]), variable.name = "rep")
guide.info.B <- data.frame(guide.info.B[!is.na(guide.info.B$value)])
guide.info.B <- merge(guide.info.B, unique(EBF1.anno.Bdev[,c(1,5)]), by.x="gene", by.y="Gene",all.x=T)
guide.info.B <- guide.info.B[order(guide.info.B$Expression_WTvsEKO, decreasing = T, na.last = F),]
guide.info.B <- guide.info.B[guide.info.B$gene %in% c(EBF1.anno.Bdev$Gene, "R26"),]
guide.info.B$condition <- factor(guide.info.B$condition, levels=c("OP9", "DL1"))

guide.info.T <- melt(setDT(guide.info.T), id.vars = colnames(guide.info.T[,1:4]), variable.name = "rep")
guide.info.T <- data.frame(guide.info.T[!is.na(guide.info.T$value)])
guide.info.T <- merge(guide.info.T, unique(EBF1.anno.Bdev[,c(1,5)]), by.x="gene", by.y="Gene",all.x=T)
guide.info.T <- guide.info.T[order(guide.info.T$Expression_WTvsEKO, decreasing = T, na.last = F),]
guide.info.T <- guide.info.T[guide.info.T$gene %in% c(EBF1.anno.Bdev$Gene, "R26"),]
guide.info.T$condition <- factor(guide.info.T$condition, levels=c("OP9", "DL1"))

#Relative to R26
R26.mean.B  <- guide.info.B[guide.info.B$gene=="R26",] %>% group_by(condition) %>% summarise_at(vars(value), list(mean = mean))
guide.info.B$FC <- ifelse(guide.info.B$condition =="OP9", guide.info.B$value/R26.mean.B[R26.mean.B$condition=="OP9",]$mean, guide.info.B$value/R26.mean.B[R26.mean.B$condition=="DL1",]$mean)
guide.info.B$name <- paste0(guide.info.B$gene, "_",guide.info.B$guide)

R26.mean.T  <- guide.info.T[guide.info.T$gene=="R26",] %>% group_by(condition) %>% summarise_at(vars(value), list(mean = mean))
guide.info.T$FC <- ifelse(guide.info.T$condition =="OP9", guide.info.T$value/R26.mean.T[R26.mean.T$condition=="OP9",]$mean, guide.info.T$value/R26.mean.T[R26.mean.T$condition=="DL1",]$mean)
guide.info.T$name <- paste0(guide.info.T$gene, "_",guide.info.T$guide)

#plot
source("script/general/r_functions/TG_theme.R")
guide.plot.B <- ggboxplot(guide.info.B, x = "gene", y = "FC", fill = "Expression_WTvsEKO",  color= "black", outlier.shape = NA)+
  ...
  stat_compare_means(label = "p.signif",  ref.group = "R26", hide.ns = TRUE, label.y = 6)+
  TG_theme()+xlab("") + ylab("Relative B cell count")+facet_grid(condition~.)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))+ylim(0,8.5)
ggsave(path="output/plots/figure3/", filename="Guide_plot_B.pdf", plot = guide.plot.B, device = "pdf", dpi=300, height=9, width=7)

guide.plot.T <- ggboxplot(guide.info.T, x = "gene", y = "FC", fill = "Expression_WTvsEKO",  color= "black", outlier.shape = NA)+
  geom_hline(yintercept =1, linetype = 2)+ 
  scale_fill_gradientn(colours=c("#1f426a","#cbcad7",'white','white',"#dcccdf", "#93302f"),breaks=c(-10, -5,0,5, 10), values=scales::rescale(c(-10,-log2(1.5)-0.01, -log2(1.5), log2(1.5), log2(1.5)+0.01, 10)),limits=c(-12, 12))+
  stat_compare_means(label = "p.signif",  ref.group = "R26", hide.ns = TRUE, label.y = 6)+
  TG_theme()+xlab("") + ylab(" Relative T cell count")+facet_grid(condition~.)+
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+theme(panel.spacing = unit(2, "lines"))+ylim(0,8.5)
ggsave(path="output/plots/figure3/", filename="Guide_plot_T.pdf", plot = guide.plot.T, device = "pdf", dpi=300, height=9, width=7)
```
...
### 3.5.2 Motif analysis for revision

Save data
```{r}
enh.motif <- unique(rbind(enh.p[,c(1,3,4)],enh.d[,c(1,3,4)]))
enh.motif$atac_group2 <- ifelse(enh.motif$atac_group %in% c("NEW", "UP"), "UP", ifelse(enh.motif$atac_group %in% c("LOST", "DOWN"), "DOWN", "NODIFF"))
enh.motif$group <- paste(enh.motif$atac_group2, enh.motif$Ebf1, sep="_")

write.table(enh.motif[,c(1,5)], "output/data/figure3/motif/Enh_Diff_Ebf1.txt", sep="\t", quote = F, col.names = T, row.names = F)
```

Run gimme maelstrom
```{bash engine.opts='-l'}
cd output/data/figure3/motif
source /opt/anaconda3/bin/activate gimme

gimme maelstrom Enh_Diff_Ebf1.txt mm10 Enh_Diff_Ebf1_motifs
```

Heatmap visualization
```{r}
source("script/general/r_functions/gimme_cluster.R")
gimme_path <- paste0(getwd(), "/output/data/figure3/motif/Enh_Diff_Ebf1_motifs/")
bb_gimme_motif_heatmap <- gimme_cluster(gimme_path, cutoff=3.5, cluster_row=T, cluster_col=T)

pdf(file=paste0("output/plots/figure3/","Gimme_motifs_Ebf1.pdf"), width = 6, height = 4)
bb_gimme_motif_heatmap
dev.off()
```
